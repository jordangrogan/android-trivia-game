...
// any code to initialize event listeners, etc.
        ...
        addButton.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                // send information back to activity using an intent (activity's onActivityResult method must be public)
                StartActivity activity = (StartActivity) getActivity();
                Intent intent = new Intent();
                intent.putExtra("word", word);
                intent.putExtra("definition", defn);
                activity.onActivityResult(StartActivity.REQ_CODE_ADD_WORD, Activity.RESULT_OK, intent);
                dismiss();
            }
        });
